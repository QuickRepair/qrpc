#include "server/reactor/epoll/EpollReactorImplementation.h"
											   ...
											   EventType et)
{
	...
	if (propertyIt != m_handles.end())
	{
		...
		else if (et & DISCONNECT_EVENT)
			originalTypes = (originalTypes & DISCONNECT_EVENT) ? (originalTypes & ~DISCONNECT_EVENT) : (originalTypes | DISCONNECT_EVENT);
